...
function predict_nt(θ_t)
    ...
    vlT2=[dis*norm(vl2[i][:,1])^2+T*(vl2[i])+Array_chain([vl2[i];(Vel2[i]-np1)*ones(1,θ_l)],ann,pn)/scale_f for i in 1:length(Vel2)]
...
function lt_pp_n_u(θ_t,Vel2) # This function gives phase portrait of the transformed system from the normal form (unstable LCO)
    ...
    vlT=[dis*norm(vl2[i][:,1])^2+T*(vl2[i])+Array_chain([vl2[i];(Vel2[i]-np1)*ones(1,θ_l)],ann,pn)/scale_f for i in 1:length(Vel2)]
    vlT
end
...
function lt_b_dia(θ_t,ind)
    ...
    dis=transpose([p5*ones(θ_l) p6*ones(θ_l)])/scale_f_l
    vlT=[dis*norm(vl[i][:,1])^2+T*(vl[i])+Array_chain([vl[i];(Vel[i]-U₀)*ones(1,θ_l)],ann,pn)/scale_f for i in 1:length(Vel)]
    vlT2=[dis*norm(vl[i][:,1])^2+T*(vl2[i])+Array_chain([vl2[i];(Vel[i]-U₀)*ones(1,θ_l)],ann,pn)/scale_f for i in 1:length(Vel)]
