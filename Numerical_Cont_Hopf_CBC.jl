using DifferentialEquations, NLsolve, Plots, Statistics, LinearAlgebra, StaticArrays, PGFPlotsX,LaTeXStrings
using ForwardDiff
#=
...
function amp_LCO(U,ind) #Extract amplitude from collocation points
    l1=length(U)
...
function flutter_eq_CBC(u, p, t) #  Equation of motion of Flutter model
    μ=p[1]
    ind1=1;ind2=3;
...
function continuation_flutter(tol,N,sp,sU,ds) #Get stable LCO from the ODE numerical solutions- initial collocation points
    Kp=0;Kd=0;μ=sU+0.02;p=[μ,0,0,0,0,0];ee=1e-3;
    eq=flutter_eq_CBC;tol=1e-7;
    jeq=(u,p,t) -> ForwardDiff.jacobian(u -> eq(u,p,t), u)
...
function continuation_CBC(tol,ctol,N,sp,sU,ds,nh) #Continuation of CBC zero problem
    ...
    # This example is assumed that we start our CBC continuation from the measured stable responses near the saddle-node bifurcation
    lp=9;s0.P[lp]
    uu=s0.V[lp];p0=[s0.P[lp],0,0,0,0,0]
    g=get_sol(uu,6,N,1,3)
    d=LS_harmonics(g.r,g.t,1,nh)
    p1=vcat([s0.P[lp],Kp,Kd],d.coeff)
    ...
    d=LS_harmonics(g.r,g.t,1,nh)
    c=transpose(d.coeff)
    p2=vcat([s0.P[lp],Kp,Kd],d.coeff)
    s2=LCO_solve(uu[1:end-1],uu[end],eq,jeq,p2,1e-3,tol)
    cbc_u2=get_CBC_u(p2)

    dv=cbc_u2-cbc_u1;dv=dv/norm(dv);pu=cbc_u2;
    aa=-dv[end] # Branch swithching to pass the saddle-node bifurcaiton
    ...
    for i in 2:sp
        cs=Newton_CBC(u1,T1,eq,jeq,p,ee,z_tol,ctol,1,3,dv,pu,ds)
        #cs=Newton_CBC(cs.u1,cs.T1,eq,jeq,cs.p,ee,z_tol,ctol/100,1,3)
        P[i]=cs.p[1]
        H[i]=cs.p[4:end]
        U[i]=[cs.u1;cs.T1]
        #U[i]=[cs.s.u[1:end-1];cs.s.u[end]]
        dv=get_CBC_u(cs.p)-pu;dv=dv/norm(dv);
        pu=get_CBC_u(cs.p);
        nu=pu+ds*dv;p=get_CBC_p(nu,Kp,Kd,N)
        uu=get_dv_u(cs.u1,cs.T1,cpu,ds)
...
sp=33;tol=1e-10;ctol=1e-9;N=100;sU=15.1;ds=0.1;nh=10;
...
lp=10
uu=s_cbc.U[lp];p0=[s_cbc.P[lp]]
jeq=(u,p,t) -> ForwardDiff.jacobian(u -> eq(u,p,t), u)
s=LCO_solve(uu[1:end-1],uu[end],eq,jeq,p0,1e-3,1e-10)
g=get_sol(s.u,6,N,1,3)

nu1=Monodromy_compute(eq,s.u,p0,N,[1,3])
r1=real(nu1)
i1=imag(nu1)
c=s_cbc.H[lp]
Kp=-100;Kd=-100;p=vcat([s_cbc.P[lp],Kp,Kd],c)
s2=LCO_solve(uu[1:end-1],uu[end],eq,jeq,p,1e-3,1e-10)
g2=get_sol(s2.u,6,N,1,3)
nu2=Monodromy_compute(eq,s2.u,p,N,[1,3])
r2=real(nu2)
i2=imag(nu2)
...
Coordinates(vcat(g2.u[:,1],g2.u[1,1]),vcat(g2.u[:,3],g2.u[1,3]))
),
LegendEntry("PP-CBC")
)
